...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			// 获取刷新的bean容器（创建当前BeanFactory容器对象）
			// 同时加载XML配置文件的属性值到当前工厂，加载BeanDefinition
			...
			try {
				...
				// 子类覆盖方法做额外的处理，此处我们自己一般不做任何拓展工作，但可以查看web中的代码，是有具体实现的
				postProcessBeanFactory(beanFactory);

				StartupStep beanPostProcess = this.applicationStartup.start("spring.context.beans.post-process");
				// Invoke factory processors registered as beans in the context.
				// 调用在上下文中注册为BeanFactoryPostProcessor Bean工厂后置处理器
				// 实例化并且开始执行BeanFactoryPostProcessors 的Bean对象
				// 调用各种beanFactory处理器
				...
				// 在所有注册的bean种查找listener bean，注册到消息广播器中
				registerListeners();
				...
				// 最后一步：完成刷新过 程，通知生命周期处理器lifecycleProcessor刷新过程，同时发出ContextRefreshEvent通知别人
				finishRefresh();
			}
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 可以在这里为bean的属性转换，比如Date类型转String，省市区3个字段转成1个字段
		beanFactory.addPropertyEditorRegistrar(new ResourceEditorRegistrar(this, getEnvironment()));

		// Configure the bean factory with context callbacks. 使用上下文回调配置 Bean 工厂

		// 添加一个BeanPostProcessor ApplicationContextAwareProcessor此类用来完成某些Aware对象的注入
		...
		// 类加载织入则指通过特殊的类加载器，在类字节码加载到JVM时，织入切面
		// 运行期织入则是采用cglib和jdk进行切面织入
		// aspectJ提供了两种织入的方式，第一种是通过特殊编译器，在编译器，将aspectJ语言编写的切面类织入到java类中，
		// 第二种是类加载器织入，就是下面的load time weaving
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		// 获取到当前应用程序上下文的beanFactoryPostProcessor变量的值，并且实例化调用执行所有已经注册的beanFactoryPostProcessor
		// 默认情况下，通过getBeanFactoryPostProcessors()来获取已经注册的BFPP，但是默认是空的，那么问题来了，如果你想拓展，怎么进行拓展工作？
	...
	protected void registerListeners() {
		...
		// 从容器中获取所有实现了ApplicationListener接口的bdName
		// 放入ApplicationListener集合中
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// Register a default embedded 嵌入式 内置的 value resolver if no BeanFactoryPostProcessor
		...
		// 冻结所有的beanDefinition，说明注册的bean定义信息将不被修改或任何进一步的处理
	 ...
	 * 该方法 refresh() 在任何其他初始化工作之前调用。
	 * 子类将创建一个新的 Bean 工厂并保存对它的引用，或者返回它所持有的单个 BeanFactory 实例。
