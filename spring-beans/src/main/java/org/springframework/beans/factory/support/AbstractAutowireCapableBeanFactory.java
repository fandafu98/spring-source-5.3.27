...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	public Object applyBeanPostProcessorsBeforeInitialization(Object existingBean, String beanName)
		...
		for (BeanPostProcessor processor : getBeanPostProcessors()) {
			// postProcessBeforeInitialization：在任何Bean初始化回调之前
			// 将此BeanPostProcessor应用到给定的新Bean实例。Bean已经填充了属性值。返回的Bean实例可能是原始Bean的包装
			// 默认实现按原样返回给定的Bean
		...
		// 返回经过所有BeanPostoProcess对象的后置处理器的层层包装后的result
		return result;
	}
	...
	protected Object doCreateBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		// ========================= 以上所有的关键的准备工作已经完成，下面开始具体初始化赋值的步骤 =========================
		/**
		 * 我们在获取完整对象的时候，分为了两个步骤，实例化和初始化
		 * 初始化环境的填充属性阶段有没有可能会改变当前对象的地址空间？不会
		 *
		 * 但是，我们不得不考虑另外一种情况
		 * spring在使用的时候配置aop，需要生成具体的代理对象
		 * 1.生成代理对象之前，要不要生成普通对象 要
		 * 2.我们能否确定什么时候会调用具体的对象？无论是普通对象还是代理对象  不能
		 * 3.有没有可能，在spring中同时存在同一个beanName的普通对象和代理对象
		 *  如果有的话，那么我在调用的时候根据beanName来获取对象，此时我要获取到两个对象吗？那么我应该用哪个？
		 *  我应该分块存储。如果是这样的话，我代理半成品是干嘛的？
		 *  那么可以创建两个缓存，分别存储普通对象和代理对象
		 * 4.这两个缓存，我优先获取哪个？
		 *  优先获取代理对象，然后才是普通对象
		 *  代理对象中应该包含了普通对象的所有功能，如果一个对象需要被代理了
		 *  那么此时普通对象就可以不存在了，只要代理对象即可
		 *  也就是说在后续创建代理对象之后，就可以使用代理对象来覆盖普通对象，普通对象可以不存在了
		 *
		 *  回到第二个问题，什么时候要调用对象？
		 *  我在调用对象的时候，如果检测到当前对象需要被代理，那么直接创建代理对象覆盖即可
		 *  那我怎么能够在随时需要的时候创建代理对象呢？
		 *  类似于观察者模式，但不是。我传递进去一个生成代理对象的匿名内部类（lambda），当需要调用的时候，直接调用匿名内部类生成代理对象，就是类似于回调机制
		 *
		 ...
		 * 如果不需要，在生成完整对象之后可以把三级缓存中lambda表达式给清除掉（往一级缓存中放成品对象的时候就可以清除）
		 *
		 * 6.我在什么时候要生成具体的代理对象？
		 *  a.在进行属性注入的时候，调用该对象生成的时候检测是否需要被代理，如果需要，直接创建代理对象
		 *  b.在整个过程中，没有其他的对象有当前对象的依赖，那么在生成最终的完整对象之前生成代理对象即可（BeanPostProcessor中的after方法）
		 *
		 */
		...
		try {
			// 填充bean的属性，将各个属性值注入，其中可能存在依赖于其他bean的属性，则会递归初始化依赖的bean
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		// 否是"synthetic"，一般是指只有AOP相关的pointCount配置或者Advice配置才会将synthetic设置为true
		// 如果mbd是不是"synthetic" 且 工厂拥有InstantiationAwareBeanPostProcessors
		if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
			// 遍历工厂中的BeanPostProcessor
			// 如果bp是InstantiationAwareBeanPostProcessors实例
			for (InstantiationAwareBeanPostProcessor bp : getBeanPostProcessorCache().instantiationAware) {
		...
		// PropertyValues:包含以一个或多个PropertyValue对象的容器，通常包含针对特定目标Bean的一次更新
		// 如果mdb有PropertyValues就获取其PropertyValues
		PropertyValues pvs = (mbd.hasPropertyValues() ? mbd.getPropertyValues() : null);

		// 获取RootBeanDefinition的自动装配模式
		int resolvedAutowireMode = mbd.getResolvedAutowireMode();
		// 如果自动装配模式为按名称自动装配bean属性或者按类型自动装配bean属性
		if (resolvedAutowireMode == AUTOWIRE_BY_NAME || resolvedAutowireMode == AUTOWIRE_BY_TYPE) {
			...
			if (resolvedAutowireMode == AUTOWIRE_BY_NAME) {
				// 通过beanWrapper的PropertyDescriptor属性名，查找出对应的Bean对象，将其添加到newPvs中
				autowireByName(beanName, mbd, bw, newPvs);
			}
			// Add property values based on autowire by type if applicable.
			// 根据autowire的类型（如适用）添加属性值
			if (resolvedAutowireMode == AUTOWIRE_BY_TYPE) {
				// 通过beanWrapper的PropertyDescriptor属性类型，查找出对应的Bean对象，将其添加到newPvs中
		...
		// 如果工厂拥有InstantiationAwareBeanPostProcessor
		if (hasInstAwareBpps) {
			// 如果pvs为null
			if (pvs == null) {
			...
			// 遍历工厂内的所有后置处理器
			for (InstantiationAwareBeanPostProcessor bp : getBeanPostProcessorCache().instantiationAware) {
				// postProcessProperties：在工厂将给定的属性值应用到给定Bean之前，对它们进行后置处理，不需要任何属性扫描符，该回调方法在未来的版本会被删掉
		...
		if (needsDepCheck) {
			if (filteredPds == null) {
				// 从beanWrapper提取一组经过筛选的PropertyDescriptor，排除忽略的依赖项或忽略项上的定义的属性
				filteredPds = filterPropertyDescriptorsForDependencyCheck(bw, mbd.allowCaching);
			}
			// 检查依赖项，主要检查pd的setter方法需要赋值时，pvs中有没有满足其pd的需求的属性值可供其复制
	...
	protected void autowireByName(
		...
		// 获取bw中有setter方法 && 非简单类型属性 & mbd的PropertyValues中没有该pd的属性名的 PropertyDescriptor属性名数组
	...
	protected void autowireByType(
		...
		for (String propertyName : propertyNames) {
			try {
				// PropertyDescriptor：标识JavaBean类通过存储器导出一个属性
				...
				// 不要尝试按照类型自动装配对象：永远是有意义的，即使它在技术上是一个不满意，复杂属性
				// 如果pd的属性值类型不是Object
				if (Object.class != pd.getPropertyType()) {
					...
					// eager为true时会导致初始化lazy-init单例和由FactoryBeans（或带有"factory-bean"引用工厂方法）创建的对象以进行类型检查
	...
	protected Object initializeBean(String beanName, Object bean, @Nullable RootBeanDefinition mbd) {
		if (System.getSecurityManager() != null) {
			...
			// 执行各种aware接口，BeanNameAware，BeanClassLoaderAware，BeanFactoryAware
			invokeAwareMethods(beanName, bean);
		}

		Object wrappedBean = bean;
		// 如果mbd不为null || mbd不是"synthetic"。一般是指只有AOP相关的pointCut配置或者Advice配置才会将synthetic设置为true
		if (mbd == null || !mbd.isSynthetic()) {
			// 将BeanPostProcessors应用到给定的现有Bean实例，调用它们的postProcessorsBeforeInitialization
		...
		if (mbd == null || !mbd.isSynthetic()) {
			...
			// BeanPostProcessors应用到给定的现有Bean实例，调用它们的postProcessAfterInitialization方法
	...
	protected void invokeInitMethods(String beanName, Object bean, @Nullable RootBeanDefinition mbd)
		...
		// InitializingBean：当Bean的所有属性都被BeanFactory设置好后，Bean需要执行相应的接口，例如执行自定义
